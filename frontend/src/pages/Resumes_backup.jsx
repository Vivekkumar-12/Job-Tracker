...
const Resumes = () => {
  ...
  const handleSaveEditCoverLetter = () => {
    if (!editCoverLetterName || !editingCoverLetter) return;
    setCoverLetters((prev) =>
      prev.map((letter) =>
        letter.id === editingCoverLetter.id
          ? { ...letter, name: editCoverLetterName, lastModified: new Date().toISOString().split('T')[0] }
          : letter
      )
    );
 ...
 onClick={() => handleEditCoverLetter(letter)}>
                            ...
                            <DropdownMenuItem className="text-destructive" onClick={() => handleDeleteCoverLetter(letter.id)}
                        >
                          <Eye className="w-4 h-4" />
                        </Button>
                        <Button
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input
                value={editCoverLetterName}
                onChange={(e) => setEditCoverLetterName(e.target.value)}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpenEditCoverLetter(false)}>Cancel</Button>
            <Button onClick={handleSaveEditCoverLetter}>Save Changes</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
